...
class PacketFunction(enum.IntEnum):
    # 可通过串口实现的控制功能(the control function realized through interface)
    PACKET_FUNC_SYS = 0
    PACKET_FUNC_LED = 1  # LED控制(LED control)
    PACKET_FUNC_BUZZER = 2  # 蜂鸣器控制(buzzer control)
    PACKET_FUNC_MOTOR = 3  # 电机控制(motor control)
    PACKET_FUNC_PWM_SERVO = 4  # PWM舵机控制, 板子上从里到外依次为1-4(PWM servo control, with the board numbered from inside to outside as 1-4)
    ...
    PACKET_FUNC_SBUS = 9  # 航模遥控获取(obtain remote control of model aircraft)
...
class PacketReportKeyEvents(enum.IntEnum):
    # 按键的不同状态(the different state of key)
...
class Board:
    ...
    def set_oled_text(self, line, text):
        data = [line, len(text)] # 子命令为 0x01 设置 SSID, 第二个字节是字符串长度，该长度包含'\0'字符串结束符(the subcommand is 0x01 to set the SSID. The second byte is the length of the string, including the '\0' string termination character)
