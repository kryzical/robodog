...
class PuppyWalkBackwardNode(Node):
    def __init__(self):
        super().__init__('puppy_walk_backward')
        ...
        self.walking_duration = 15.0  # Total walking time in seconds
        
        # Create timer for walking sequence
        self.walk_timer = self.create_timer(0.1, self.walk_sequence)
        self.start_time = time.time()
        
        self.get_logger().info('Puppy walk backward node started')
    ...
    def calculate_walking_positions(self, phase):
        ...
        # Calculate step offsets (negative for backward walking)
